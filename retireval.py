...
class CXRDatasetForRetrieval(Dataset):
    ...
    def __init__(self, config, args, split='train'):
        ...
        print("load data index information from: {}".format(self.data_path_cleaned))
        ...
        print(f"{self.split}: {self.img_num}")
...
def trainRetrieval(model, epochs, train_data_loader, save_path='./checkpoints/report_retrieval.model', save_freq=30):
    ...
    for epoch in range(epochs):
        ...
        if (epoch+1) % save_freq == 0:
            ...
            print(f'save to {_save_path}')
        print("epoch: {}, using time: {},loss: {}".format(epoch, time.time() - t, total_loss / n) )
    return
...
def testRetrieval(model, test_data_loader, output_file='./results/retrieval/test_result.pt'):
    ...
    print("Prediction results and labels saved to {}".format(output_file))
...
def RetrievalTask(args):
    ...
    if args.ckpt_path is not None:
        print(f"load models from {args.ckpt_path}")
    ...
    print("total number of train samples: {}, test samples: {}".format(len(train_dataset), len(test_dataset)))
    ...
    if args.do_train:
        ...
        trainRetrieval(model, epochs=epochs, train_data_loader=train_dataloader,
                       save_path=f"./checkpoints/IUXray_report_retrieval_model_{args.conv}-{'pretrained' if args.pretrained else ''}-{time_tmp}",  
    ...
    if args.do_rank:
        print("Start ranking the result.")
        ...
        print("Evaluation results saved to {}.".format(result_file))
    return
...
if __name__ == '__main__':
    args = parse_args()
    time_tmp = time.asctime(time.localtime(time.time())).replace(':', '-')
    print(f'Starting IUXray-CXR_retrieval with {args.conv} at {time_tmp}')
